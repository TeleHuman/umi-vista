...
VISTA is the release-facing policy type for VISTA checkpoints. It uses the same model architecture as the imported OpenPI-style implementation while keeping checkpoint metadata and CLI usage separate from legacy policy names.
{% elif model_name == "sac" %}
