...
_XML = _HERE / "model" / "acone" / "desk_scene.xml"
_MAPPING = [{'mj_name': 'left_joint1', 'robot_index': 0, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'left_joint2', 'robot_index': 1, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'left_joint3', 'robot_index': 2, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'left_joint4', 'robot_index': 3, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'left_joint5', 'robot_index': 4, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'left_joint6', 'robot_index': 5, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'right_joint11', 'robot_index': 6, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'right_joint12', 'robot_index': 7, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'right_joint13', 'robot_index': 8, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'right_joint14', 'robot_index': 9, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'right_joint15', 'robot_index': 10, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'right_joint16', 'robot_index': 11, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'left_joint7', 'robot_index': 12, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'left_joint8', 'robot_index': 12, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'right_joint17', 'robot_index': 13, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0},
            {'mj_name': 'right_joint18', 'robot_index': 13, 'scale': 1.0, 'offset': 0.0, 'sign': 1.0}]
...
class AconeMink:
    def __init__(self):
        ...
        collision_pairs = [(["left_hand_collision1"], ["base_collision"]),
                           (["right_hand_collision1"], ["base_collision"]),
                           (["sigularity_base_collision1"], ["sigularity_ee_collision1"]),
                           (["sigularity_base_collision2"], ["sigularity_ee_collision2"])]
    ...
    def apply_mapped_qpos(self, q_mapped, sync_mocap: bool = False):
        ...
        if q.shape[0] < 1:
            return
        # Structure:
        # First 12 values: 6 (left arm) + 6 (right arm)
        ...
        with self._mj_lock:
            ...
            if q.shape[0] >= 14:
                left_grip_pct = float(np.clip(q[12], 0.0, 100.0))
                right_grip_pct = float(np.clip(q[13], 0.0, 100.0))
                # Percentage -> m (consistent with set_real_qpos)
                left_grip_m = left_grip_pct * 0.001 / 2.0
                right_grip_m = right_grip_pct * 0.001 / 2.0
                # Mapping indices: 12,13 for left fingers; 14,15 for right fingers
                if len(_MAPPING) >= 16:
                    try:
                        jid_l1 = self.model.joint(_MAPPING[12]['mj_name']).id
                        jid_l2 = self.model.joint(_MAPPING[13]['mj_name']).id
                        self.data.qpos[jid_l1] = left_grip_m
                        self.data.qpos[jid_l2] = left_grip_m
                        jid_r1 = self.model.joint(_MAPPING[14]['mj_name']).id
                        jid_r2 = self.model.joint(_MAPPING[15]['mj_name']).id
    ...
    def get_sim_feedback(self):
        ...
        right_grip_fb = 0.0
        if qarr.shape[0] >= 16:
            left_grip_fb = float((qarr[12] + qarr[13]) * 1000) # convert to mm
            right_grip_fb = float((qarr[14] + qarr[15]) * 1000) # convert to mm
        feedback = {
            'qpos': {
                'left_arm': qarr[0:6].copy(),
                'right_arm': qarr[6:12].copy(),
                'left_gripper': [left_grip_fb],
                'right_gripper': [right_grip_fb],
            },
            'qvel': {
                'left_arm': varr[0:6].copy(),
                'right_arm': varr[6:12].copy(),
            }
        }
        return feedback
    ...
    def check_collision_pairs(self):
        ...
        with self._mj_lock:
            ...
            if _fwd is not None:
                _fwd(self.model, self.data)
            pairs = [(["left_hand_collision1"], ["base_collision"]),
                     (["right_hand_collision1"], ["base_collision"])]
...
if __name__ == "__main__":
    r1pro = AconeMink()
    r1pro.test_traj(duration=10.0, viewer=True, hand_amp=0.0, hand_freq=0.0)
