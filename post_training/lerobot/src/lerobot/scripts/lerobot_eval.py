...
def _to_numpy_image(x):
    """Convert image-like tensor/array to HWC uint8 RGB."""
    if isinstance(x, torch.Tensor):
        x = x.detach().cpu().numpy()

    x = np.asarray(x)

    # remove batch dim if shape is (B,H,W,C) or (B,C,H,W)
    if x.ndim == 4:
        x = x[0]
...
def save_observation_images(observation, out_dir, prefix="reset"):
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    print("\n[save_observation_images] observation keys:")
    for k, v in observation.items():
        print(" ", k, type(v))

    # case 1: raw env obs: {"pixels": {"wrist_image": ...}}
    if "pixels" in observation:
        for name, img in observation["pixels"].items():
            img = _to_numpy_image(img)
            path = out_dir / f"{prefix}_pixels_{name}.png"
            cv2.imwrite(str(path), img[..., ::-1])  # RGB -> BGR
            print("saved:", path, img.shape, img.dtype)

    # case 2: LeRobot obs after preprocess: {"observation.images.wrist_image": tensor}
    for key, val in observation.items():
        if "image" in key:
            img = _to_numpy_image(val)
            safe_key = key.replace("/", "_").replace(".", "_")
            path = out_dir / f"{prefix}_{safe_key}.png"
            cv2.imwrite(str(path), img[..., ::-1])
            print("saved:", path, img.shape, img.dtype)

    # also save state if present
    for key, val in observation.items():
        if "state" in key or "agent_pos" in key:
            arr = val.detach().cpu().numpy() if isinstance(val, torch.Tensor) else np.asarray(val)
            np.save(out_dir / f"{prefix}_{key.replace('.', '_')}.npy", arr)
            print("saved state:", key, arr.shape, arr.dtype)
...
def rollout(
    ...
    #     observation,
    #     out_dir="/data/guolinzheng/test/libero_eval_first_obs/raw_after_env_reset_v2",
    #     prefix="env_reset",
    # )
    ...
    if policy.config.use_delta_action:
        ...
        mask[7] = False
        absolute_action_transform = AbsoluteActionTransform(mask)
    ...
    while not np.all(done) and step < max_steps:
        ...
        # temp_dict = {"observation.state": observation["observation.state"].clone().cpu()}
        # # import ipdb;ipdb.set_trace()
        # observation = preprocessor(observation)
        # del observation['observation.images.unused_image'] ##umi(5)
        # with torch.inference_mode():
        #     action = policy.select_action(observation)
        # action = postprocessor(action)
        # if policy.config.use_delta_action:
        #     temp_dict["action"] = action.unsqueeze(-2).to("cpu")
        #     action = absolute_action_transform(temp_dict)["action"][0]
        observation = preprocess_observation(observation)
        if return_observations:
            all_observations.append(deepcopy(observation))
        ...
        # Starting state for the current chunk. The full chunk delta-to-absolute conversion uses this state.
        chunk_start_state = observation["observation.state"].clone().cpu()
        # import ipdb;ipdb.set_trace()
        observation = preprocessor(observation)
        if "observation.images.unused_image" in observation:
            del observation["observation.images.unused_image"]

        # Predict a new chunk only when the queue is empty
        if len(abs_action_queue) == 0:
            with torch.inference_mode():
                action_chunk = policy.predict_action_chunk(observation)
            # Keep only the n_action_steps to execute
            ...
            if policy.config.use_delta_action:
                temp_dict = {
                    "observation.state": chunk_start_state,
                    "action": action_chunk.to("cpu"),
                }
                action_chunk = absolute_action_transform(temp_dict)["action"]
                # import ipdb;ipdb.set_trace()
            # action_chunk: [B, T, 8]
            # Each queue element: [B, 8]
            abs_action_queue.extend(action_chunk.transpose(0, 1))

        # Use the already processed absolute action
        action = abs_action_queue.popleft()


        # Convert to CPU / numpy.
        action_numpy: np.ndarray = action.to("cpu").numpy()
        assert action_numpy.ndim == 2, "Action dimensions should be (batch, action_dim)"
        action_numpy[:,7] = -(action_numpy[:,7]*2-1)
        # import ipdb;ipdb.set_trace()
...
###############################################################
###############################################################
#original eval code, not used anymore but kept for reference
###############################################################
#!/usr/bin/env python
